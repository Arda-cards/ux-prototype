...
import { DetailField } from '@/components/canary/atoms/detail-field/detail-field';
...
export function ItemDetailsPanel({
  ...
  const [selectedCardEid, setSelectedCardEid] = useState<string>(item.eid || '');
  ...
  const { isToastVisible, showToast, hideToast, handleUndo } = useOrderQueueToast();
  ...
  const getCardsForModal = (): ItemCard[] => {
    ...
    return cardList.map((card) => ({
      ...
      orderUnit: item.orderUnit || '',
      supplier: item.supplier || '',
      sku: card.payload.itemDetails?.internalSKU ?? card.payload.serialNumber ?? '',
      image: item.image || '',
      link: item.link || '',
      unitPrice: item.unitPrice || 0,
    }));
  ...
  const selectedCard = cardList.find((card) => card.payload.eId === selectedCardEid);
  const canAddToCart = selectedCard ? canAddToOrderQueue(selectedCard.payload.status) : false;
  ...
  const handleAddToOrderQueue = async () => {
    ...
    try {
      if (fromEl && toEl) {
        ...
        toEl.classList.add('ring-2', 'ring-[var(--base-primary)]', 'ring-offset-2', 'rounded-md');
        setTimeout(() => {
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/arda/kanban/kanban-card/query-by-item?eId=${item.eid}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${jwtToken}`,
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (cardsToDelete.length > 0) {
        const cardDeletePromises = cardsToDelete.map(async (card) => {
          try {
            ...
            console.error(`Failed to delete card ${card.payload.eId}:`, response.status, errorText);
        ...
        if (failedCards.length > 0) {
          console.warn(`Failed to delete ${failedCards.length} of ${cardsToDelete.length} cards`);
          toast.error(
        ...
        if (data.ok) {
          const cardsMessage =
            cardsToDelete.length > 0
              ? ` and ${cardsToDelete.length} card${cardsToDelete.length > 1 ? 's' : ''}`
              : '';
      ...
      window.dispatchEvent(new CustomEvent('itemDeleted', { detail: { itemId: item.eid } }));
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.ok && data.data && data.data.url) {
          ...
          console.error('Failed to print card - invalid response structure:', data);
    ...
    try {
      ...
      if (printResponse.ok) {
        ...
        if (printData.ok && printData.data && printData.data.url) {
          ...
          console.error('Failed to print label - invalid response structure:', printData);
    ...
    try {
      ...
      if (printResponse.ok) {
        ...
        if (printData.ok && printData.data && printData.data.url) {
          ...
          console.error('Failed to print breadcrumb - invalid response structure:', printData);
  ...
  return (
    <>
        ...
        >
          ...
          <div className="sticky top-0 z-40 bg-white px-4 sm:px-6 pt-4 pb-2 sm:pt-5 sm:pb-3 border-b">
            <div className="flex flex-col gap-y-1.5">
              ...
              <div className="h-px bg-[#e5e5e5]" style={{ marginInline: '-24px' }} />
              ...
              {activeTab === 'details' && (
                <div className="-mx-6 flex items-center justify-center gap-1 sm:gap-2 flex-wrap px-6 pt-1">
                  ...
                  >
                    ...
                    <span className="sm:hidden">{isPrintingAll ? 'Printing...' : 'Print'}</span>
                  </Button>
                  ...
                  >
                    ...
                    <span className="sm:hidden">{isPrintingLabel ? 'Printing...' : 'Label'}</span>
                  </Button>
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">
                      {isPrintingBreadcrumb ? 'Printing...' : 'Print breadcrumb'}
                    </span>
            ...
            {activeTab === 'details' ? (
              <>
                  ...
                  <div className="w-full max-w-[396px] flex flex-col items-center gap-2">
                    ...
                    {isLoadingCards && (
                      <div className="w-full flex justify-center items-center py-8">
                        <div className="text-sm text-[#737373]">Loading cards...</div>
                      </div>
                    )}
                        ...
                        <div className="flex-1 flex items-center justify-center -ml-12">
                          <div className="flex items-center gap-2">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => setCurrentCardIndex(Math.max(1, currentCardIndex - 1))}
                            ...
                            <span className="text-sm font-medium text-[#0a0a0a]">
                              <strong>{currentCardIndex}</strong> of <strong>{totalCards}</strong>
                            </span>
                            <Button
                              variant="outline"
                              size="sm"
                ...
                <div className="w-full px-4 sm:px-6 space-y-3 py-4 mb-6">
                  ...
                  />
                  <DetailField label="Number of cards" value={String(totalCards)} />
                </div>
              </>
            ) : (
              <ManageCardsPanel
      ...
      <OrderQueueToast isVisible={isToastVisible} onUndo={handleUndo} onClose={hideToast} />
