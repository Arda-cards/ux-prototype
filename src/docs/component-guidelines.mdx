...
For building entity-level viewers and editors, see the
[Creating Entity Viewers](?path=/docs/docs-patterns-creating-entity-viewers--docs) pattern guide.
The `createArdaEntityViewer` factory provides a standardized approach with built-in state management,
validation, and dual-layout support.
