...
<Meta title="App/Reference/Integration" />

# App Current Integration

The **App/Reference** section in the Storybook sidebar contains live page components vendored from the production `arda-frontend-app` repository. Each page renders against MSW mock services, reproducing the real application behavior without a running backend.
...
The `withFullAppProviders` decorator in `src/decorators/with-full-app-providers.tsx` wraps every story whose title starts with `App/Reference` or `App/Canary`. It composes the same provider hierarchy used in the production application:
...
Stories in the `App/Canary` section use the same provider stack as `App/Reference`, enabling side-by-side parity comparison with no additional setup. Non-full-app stories pass through the decorator unchanged.
...
The `App/Reference` section contains 12 story files with 22 total variants, organized into five sections that mirror the application&#8217;s navigation structure:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>App/Reference/Home/Dashboard</td>
      <td>
        <code>/dashboard</code>
      </td>
    ...
    <tr>
      ...
      <td>App/Reference/Reference/Items/Items Grid</td>
      <td>
        <code>/items</code>
      </td>
    ...
    <tr>
      <td>App/Reference/Reference/Items/Item Detail</td>
      <td>
    ...
    <tr>
      ...
      <td>App/Reference/Resources/Kanban Cards/Kanban Card</td>
      <td>
    ...
    <tr>
      <td>App/Reference/Resources/Kanban Cards/Mobile Device Check</td>
      <td>
    ...
    <tr>
      <td>App/Reference/Resources/Kanban Cards/Scan</td>
      <td>
        <code>/scan</code>
      </td>
      <td>Default</td>
    ...
    <tr>
      ...
      <td>App/Reference/System/Authentication/Sign In</td>
      <td>
        <code>/signin</code>
      </td>
    ...
    <tr>
      <td>App/Reference/System/Authentication/Sign Up</td>
      <td>
        <code>/signup</code>
      </td>
      <td>Default</td>
    ...
    <tr>
      <td>App/Reference/System/Authentication/Reset Password</td>
      <td>
    ...
    <tr>
      <td>App/Reference/System/Settings</td>
      <td>
        <code>/settings</code>
      </td>
    ...
    <tr>
      ...
      <td>App/Reference/Transactions/Orders/Order Queue</td>
      <td>
    ...
    <tr>
      <td>App/Reference/Transactions/Receiving</td>
      <td>
        <code>/receiving</code>
      </td>
      <td>Default</td>
...
Every `App/Reference` story carries metadata that traces it back to the production application:
...
The `App/Reference` stories are covered by three layers of automated testing:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Play functions execute assertions against rendered pages (all `App/Reference` and
        `App/Canary` story variants)
      </td>
    </tr>
    <tr>
      ...
      <td>Playwright</td>
      <td>
        Pixel-level comparison of `App/Reference` page screenshots against baselines (1% diff
        threshold)
      </td>
    </tr>
    <tr>
...
## App/Canary: Migration Workbench

The vendored code establishes the foundation for a component migration project. The **App/Canary** section (`src/canary-refactor/`) is the active working area where vendored page components are progressively refactored to use migration-track library components instead of inline production components.
...
3. Intentional deviations from the `App/Reference` reference are documented in the story JSDoc comment with an **Intentional deviations from App Current** block listing before/after values.
4. The `withFullAppProviders` decorator wraps `App/Canary` stories automatically &#8212; no additional setup is required.

Story files follow the same path and title structure as their `App/Reference` counterparts, with `App/Reference` replaced by `App/Canary`:

<table>
  <thead>
    <tr>
      <th>App/Reference original</th>
      <th>App/Canary mirror</th>
    </tr>
  </thead>
  <tbody>
    <tr>
