...
<Meta title="Docs/Storybook Structure/Dev Witness/Integration" />

# Dev Witness Integration

The **Dev Witness** section in the Storybook sidebar contains live page components vendored from the production `arda-frontend-app` repository. Each page renders against MSW mock services, reproducing the real application behavior without a running backend.
...
The `withFullAppProviders` decorator in `src/decorators/with-full-app-providers.tsx` wraps every story whose title starts with `Dev Witness` or `Canary Refactor`. It composes the same provider hierarchy used in the production application:
...
Stories in the `Canary Refactor` section use the same provider stack as Dev Witness, enabling side-by-side parity comparison with no additional setup. Non-full-app stories pass through the decorator unchanged.
...
The Dev Witness section contains 12 story files with 22 total variants, organized into five sections that mirror the application&#8217;s navigation structure:
...
<table>
  ...
  <tbody>
    <tr>
      <td rowspan="1"><strong>Home</strong></td>
      <td>Dev Witness/Home/Dashboard</td>
      <td><code>/dashboard</code></td>
      <td>Default, NotificationPanel, HelpPanel</td>
    </tr>
    <tr>
      <td rowspan="2"><strong>Reference</strong></td>
      <td>Dev Witness/Reference/Items/Items Grid</td>
      <td><code>/items</code></td>
      <td>Default, EmptyState</td>
    </tr>
    <tr>
      <td>Dev Witness/Reference/Items/Item Detail</td>
      <td><code>/item/[itemId]</code></td>
      <td>Default, AddItemForm</td>
    </tr>
    <tr>
      <td rowspan="3"><strong>Resources</strong></td>
      <td>Dev Witness/Resources/Kanban Cards/Kanban Card</td>
      <td><code>/kanban/cards/[cardId]</code></td>
      <td>Default, CardNotFound</td>
    </tr>
    <tr>
      <td>Dev Witness/Resources/Kanban Cards/Mobile Device Check</td>
      <td><code>/mobile-device-check</code></td>
      <td>Default</td>
    </tr>
    <tr>
      <td>Dev Witness/Resources/Kanban Cards/Scan</td>
      <td><code>/scan</code></td>
      <td>Default</td>
    </tr>
    <tr>
      <td rowspan="4"><strong>System</strong></td>
      <td>Dev Witness/System/Authentication/Sign In</td>
      <td><code>/signin</code></td>
      <td>Default, InvalidCredentials</td>
    </tr>
    <tr>
      <td>Dev Witness/System/Authentication/Sign Up</td>
      <td><code>/signup</code></td>
      <td>Default</td>
    </tr>
    <tr>
      <td>Dev Witness/System/Authentication/Reset Password</td>
      <td><code>/reset-password</code></td>
      <td>Default</td>
    </tr>
    <tr>
      <td>Dev Witness/System/Settings</td>
      <td><code>/settings</code></td>
      <td>Account, Companies, Appearance, Display</td>
    </tr>
    <tr>
      <td rowspan="2"><strong>Transactions</strong></td>
      <td>Dev Witness/Transactions/Orders/Order Queue</td>
      <td><code>/order-queue</code></td>
      <td>Default, EmptyQueue</td>
    </tr>
    <tr>
      <td>Dev Witness/Transactions/Receiving</td>
...
Every Dev Witness story carries metadata that traces it back to the production application:
...
The Dev Witness stories are covered by three layers of automated testing:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Play functions execute assertions against rendered pages (all Dev Witness and Canary Refactor story variants)</td>
    </tr>
    <tr>
      <td>Visual regression</td>
      <td>Playwright</td>
      <td>Pixel-level comparison of Dev Witness page screenshots against baselines (1% diff threshold)</td>
    </tr>
    <tr>
...
## Canary Refactor: Migration Workbench

The vendored code establishes the foundation for a component migration project. The **Canary Refactor** section (`src/canary-refactor/`) is the active working area where vendored page components are progressively refactored to use canary-stage library components instead of inline production components.

### How it works

1. A page is forked from its vendored counterpart in `src/dev-witness/` into a matching path under `src/canary-refactor/`.
2. Vendored sub-components are replaced one-by-one with canary equivalents (e.g., `ReadOnlyField` replacing inline `flex flex-col gap-1` blocks).
3. Intentional deviations from the Dev Witness baseline are documented in the story JSDoc comment with a **Intentional deviations from Dev Witness** block listing before/after values.
4. The `withFullAppProviders` decorator wraps Canary Refactor stories automatically &#8212; no additional setup is required.

Story files follow the same path and title structure as their Dev Witness counterparts, with `Dev Witness` replaced by `Canary Refactor`:

<table>
  <thead>
    <tr>
      <th>Dev Witness original</th>
      <th>Canary Refactor mirror</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><code>src/dev-witness/reference/items/item-detail.stories.tsx</code></td>
      <td><code>src/canary-refactor/reference/items/item-detail.stories.tsx</code></td>
    </tr>
  </tbody>
</table>
