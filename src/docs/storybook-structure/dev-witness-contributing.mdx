...
<Meta title="Docs/Storybook Structure/Dev Witness/Contributing" />
...
Each Dev Witness page story lives in `src/dev-witness/` and follows a consistent template.
...
- The `title` must start with `Dev Witness/` so the `withFullAppProviders` decorator activates. Use a minimum three-level title (`Dev Witness/Section/Page`) matching the application&#8217;s navigation structure. See the [Page Inventory](?path=/docs/docs-storybook-structure-dev-witness-integration--docs#page-inventory) for the section taxonomy.
...
**Fix:** Verify the story title starts with `Dev Witness/` or `Canary Refactor/` (case-sensitive). The `withFullAppProviders` decorator activates for both prefixes. If the title is correct, check that `MockAuthProvider` and the Redux store pre-loading are working by inspecting the browser console for auth-related errors.
