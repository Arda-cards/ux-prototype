...
<Meta title="App/Reference/Contributing" />

# App Current Contributing Guide

This page covers how to add new page stories, override MSW handlers, run the sync script locally, and debug common issues with the `App/Reference` integration.

## Adding a New Page Story

Each `App/Reference` page story lives in `src/dev-witness/` and follows a consistent template. The folder name is historical; the Storybook section label is `App/Reference`.
...
- The `title` must start with `App/Reference/` so the `withFullAppProviders` decorator activates. Use a minimum three-level title (`App/Reference/Section/Page`) matching the application&#8217;s navigation structure. See the [Page Inventory](?path=/docs/app-current-integration--docs#page-inventory) for the section taxonomy.
- The `tags` array should include `app-route:<path>` for route traceability.
- Import `@/styles/vendored/globals.css` to apply the production application&#8217;s CSS custom properties. This file uses the `theme-override` CSS layer so vendored tokens take precedence over the Nominal base theme. See the [CSS Layer System](?path=/docs/docs-css-layer-system--docs) for details.
...
**Symptom:** An `App/Reference` story renders the sign-in page instead of the expected page.

**Cause:** The auth guard detected missing or invalid auth state and redirected to `/signin`.

**Fix:** Verify the story title starts with `App/Reference/` or `App/Canary/` (case-sensitive). The `withFullAppProviders` decorator activates for both prefixes. If the title is correct, check that `MockAuthProvider` and the Redux store pre-loading are working by inspecting the browser console for auth-related errors.
...
After making intentional visual changes to an `App/Reference` page, update the Playwright screenshot baselines:
