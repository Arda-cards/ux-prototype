...
/**
 * Generic notes icon cell renderer - displays a notes icon with tooltip.
 * Shows MessageSquare icon if notes exist, or em-dash if not.
 */
export function NotesIconCell({ notes }: { notes?: string }) {
  const handleMouseEvent = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
  }, []);
...
/**
 * Generic quick actions placeholder cell renderer.
 * Shows "Actions" text placeholder with no entity dependency.
 */
export function QuickActionsPlaceholderCell() {
  const handleMouseEvent = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
  }, []);
