...
describe('ArdaSidebarNavItem', () => {
  it('renders a button with label', () => {
    renderInSidebar(<ArdaSidebarNavItem icon={Package} label="Items" />);
    const button = screen.getByRole('button', { name: /items/i });
    expect(button).toBeInTheDocument();
  });

  it('sets data-active when active', () => {
    renderInSidebar(<ArdaSidebarNavItem icon={Package} label="Items" active />);
    const button = screen.getByRole('button', { name: /items/i });
  ...
  it('renders badge when provided', () => {
    renderInSidebar(<ArdaSidebarNavItem icon={ShoppingCart} label="Orders" badge={5} />);
    expect(screen.getByText('5')).toBeInTheDocument();
  });

  it('renders string badge', () => {
    renderInSidebar(<ArdaSidebarNavItem icon={ShoppingCart} label="Orders" badge="New" />);
    expect(screen.getByText('New')).toBeInTheDocument();
  });

  it('renders dot badge when badge is true', () => {
    renderInSidebar(<ArdaSidebarNavItem icon={Package} label="Suppliers" badge={true} />);
  ...
  it('applies className prop', () => {
    renderInSidebar(<ArdaSidebarNavItem icon={Package} label="Items" className="my-custom" />);
    const button = screen.getByRole('button', { name: /items/i });
