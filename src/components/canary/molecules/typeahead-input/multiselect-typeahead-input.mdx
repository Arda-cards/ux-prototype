...
> Tokens render as `TokenChip` pills whose hints (`tokenAction`, `optionDestroy`,
> the × remove, the edit hint) are design-system tooltips — a `TooltipProvider`
> must be present in the tree, the same requirement as `Button`'s `tooltip` prop.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>Shift+Delete</code></td>
      <td>On a highlighted dropdown row, fires <code>optionDestroy</code> (keyboard twin of the hover-revealed ×).</td>
    </tr>
    <tr>
      <td><code>Shift+Space</code></td>
      <td>On a focused token, fires <code>tokenAction</code> (keyboard twin of the hover-revealed action button).</td>
    </tr>
    <tr>
      <td><code>Shift+Enter</code></td>
      <td>On a focused token with <code>editOnDoubleClick</code>, pulls the token back into the input for editing.</td>
    </tr>
  </tbody>
</table>

The hover-revealed buttons (token action, token ×, option destroy) are
deliberately not in the tab order — tabbing through every internal button
would wreck form tab flow. The shortcuts above are their keyboard
equivalents and are announced via <code>aria-keyshortcuts</code>.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>optionDestroy</code></td>
      <td><code>MultiSelectOptionDestroy</code></td>
      <td>&#8212;</td>
      <td>Per-option destroy button at the far right of each dropdown row, revealed on hover/highlight: <code>&#123; label(value), icon?, onDestroy(value), isVisible?(value) &#125;</code> (icon defaults to ×). Destructive by design: firing it does not select the option and optimistically drops the row; the caller owns removing it from the lookup source.</td>
    </tr>
    <tr>
