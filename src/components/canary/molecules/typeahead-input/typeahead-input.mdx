...
`cellEditorMode`, `clearOnFocus`, `selectOnFocus`, `clearOnEmptyBlur`, and
`maxResults`.
...
## selectOnFocus and clearOnEmptyBlur

Two independent, opt-in affordances for form fields:

- **`selectOnFocus`** &#8212; focusing highlights (selects) the current text so
  typing replaces it. The text stays visible; clicking away without editing
  changes nothing. Mutually exclusive with `clearOnFocus`, which empties the
  input instead.
- **`clearOnEmptyBlur`** &#8212; leaving the field (blur or Tab) with the text
  emptied commits the clear (`onValueChange('')`) instead of restoring the
  previous value. Form-mode blur already resolves every non-empty edit
  (perfect match, create, highlighted row); this closes the one edit that
  could not leave the field. Escape still restores, and Enter keeps its
  select-the-highlight semantics. Form mode only &#8212; ignored under
  `cellEditorMode`, where the grid owns commit semantics &#8212; and it
  resolves through the same dismissal paths as the component's default
  blur-revert (outside click, focus moving elsewhere, Tab).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>&#8212;</td>
      <td>Called when the user selects or creates a value — and, with <code>clearOnEmptyBlur</code>, with <code>''</code> when an emptied field commits its clear.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><code>boolean</code></td>
      <td><code>false</code></td>
      <td>Highlight (select) the current text on focus so typing replaces it. Mutually exclusive with <code>clearOnFocus</code>.</td>
    </tr>
    <tr>
      <td><code>clearOnEmptyBlur</code></td>
      <td><code>boolean</code></td>
      <td><code>false</code></td>
      <td>Leaving the field (blur or Tab) with the text emptied commits the clear instead of restoring the previous value. Escape still restores.</td>
    </tr>
    <tr>
