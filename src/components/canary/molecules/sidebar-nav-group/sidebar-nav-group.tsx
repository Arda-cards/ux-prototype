  ...
  return (
    <li className={cn('list-none', className)}>
      <Collapsible defaultOpen={defaultExpanded}>
        <CollapsibleTrigger className="flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm text-sidebar-text-muted hover:bg-sidebar-hover hover:text-sidebar-text-active transition-colors duration-150 motion-reduce:transition-none outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-sidebar-bg group">
          {Icon && <Icon size={18} className="shrink-0" aria-hidden="true" />}
          <span className="truncate font-medium">{label}</span>
          <ChevronRight
            size={14}
            className="ml-auto shrink-0 transition-transform duration-150 motion-reduce:transition-none group-data-[state=open]:rotate-90"
            aria-hidden="true"
          />
        </CollapsibleTrigger>
        <CollapsibleContent>
