...
describe('DetailField', () => {
  ...
  it('renders children even when value is also provided', () => {
    render(
      <DetailField label="Custom" value="ignored value">
  ...
  it('applies className prop to the root element', () => {
    const { container } = render(
      <DetailField label="Test" value="Val" className="my-custom-class" />,
    );
    expect(container.firstChild).toHaveClass('my-custom-class');
  });

  it('applies compact variant class', () => {
    const { container } = render(<DetailField label="Test" value="Val" variant="compact" />);
    expect(container.firstChild).toHaveClass('gap-0.5');
  });

  it('applies default variant (no extra gap class)', () => {
    const { container } = render(<DetailField label="Test" value="Val" variant="default" />);
  ...
  it('passes through HTML attributes to the root div', () => {
    render(<DetailField label="Test" value="Val" data-testid="detail-field" />);
    expect(screen.getByTestId('detail-field')).toBeInTheDocument();
  });

  it('label uses muted-foreground token class', () => {
    render(<DetailField label="My Label" value="Val" />);
    const label = screen.getByText('My Label');
    expect(label).toHaveClass('text-muted-foreground');
  });

  it('value uses foreground token class', () => {
    render(<DetailField label="My Label" value="My Value" />);
