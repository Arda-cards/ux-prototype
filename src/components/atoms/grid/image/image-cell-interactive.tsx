...
export interface ArdaImageCellInteractiveProps extends AtomProps<string> {}

export function ArdaImageCellInteractive({
  value,
  onChange,
  onComplete,
  onCancel,
  ...
  errors,
  editable,
}: ArdaImageCellInteractiveProps) {
  const effectiveMode: AtomMode = editable === false ? 'display' : mode;

  if (effectiveMode === 'display') {
    return <ArdaImageCellDisplay value={value} />;
  }

  return (
    <ImageCellInlineEditor
      value={value}
      onChange={onChange}
