...
const NEW_SUBLOCATION_OPTION = '__new_sublocation__';

interface SublocationTypeaheadProps {
  value: string;
...
export function SublocationTypeahead({
  value,
  onChange,
  ...
  const displayOptions = useMemo(() => {
    ...
    if (trimmed && !options.some((o) => o.toLowerCase() === trimmed.toLowerCase())) {
      list.push({ value: NEW_SUBLOCATION_OPTION, isNew: true });
    }
    return list;
  }, [options, inputValue]);

  const fetchSublocations = useCallback(async (search: string) => {
    if (!search.trim()) {
      setOptions([]);
      return;
    }
    ...
    setIsLoading(true);
    try {
      const results = await lookupSublocations(search.trim());
      setOptions(results);
    } catch {
      setOptions([]);
    } finally {
  ...
  const handleFocus = useCallback(() => {
    ...
    if (inputValue.trim()) {
      fetchSublocations(inputValue);
    }
    setIsOpen(true);
    setHighlightedIndex(-1);
  }, [inputValue, fetchSublocations]);
  ...
  const selectValue = useCallback(
    (val: string) => {
      const final = val === NEW_SUBLOCATION_OPTION ? inputValue.trim() : val;
      ...
      if (isOpen && displayOptions.length > 0) {
        ...
        if (e.key === 'Enter') {
          ...
          if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
            ...
            selectValue(NEW_SUBLOCATION_OPTION);
          }
          return;
        }
        if (e.key === 'Escape') {
        ...
        if (e.key === 'Tab') {
          if (cellEditorMode) {
            if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
              ...
              const final = selected === NEW_SUBLOCATION_OPTION ? inputValue.trim() : selected;
              if (final) {
                onChange(final);
                setInputValue(final);
              }
              ...
              inputValue.trim() && (
                ...
                >
                  ...
                  <span className="text-sm text-gray-900">
                    New sub-location: &quot;{inputValue.trim()}&quot;
                  </span>
                </div>
              )
            ) : (
