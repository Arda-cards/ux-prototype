import { SublocationCellEditor } from './SublocationCellEditor';

jest.mock('@/lib/ardaClient', () => ({ lookupSublocations: jest.fn().mockResolvedValue([]) }));
...
describe('SublocationCellEditor', () => {
  let editor: SublocationCellEditor;
  let params: {
    value: string;
  ...
  beforeEach(() => {
    editor = new SublocationCellEditor();
    params = {
      value: 'Bin A',
      stopEditing: jest.fn(),
    };
  ...
  it('getGui returns a div element after init', () => {
    ...
    expect(gui.className).toContain('ag-cell-edit-sublocation');
  });
  ...
  it('getValue returns input value when input is present', () => {
    ...
    expect(editor.getValue()).toBe('Shelf B');
  });

  it('getValue returns committedValue when no input present', () => {
    editor.init(params as never);
    expect(editor.getValue()).toBe('Bin A');
  });
  ...
  it('focusOut updates committedValue from input value', () => {
    ...
    const gui = editor.getGui();
    const input = document.createElement('input');
    input.value = '  Drawer C  ';
    gui.appendChild(input);
    editor.focusOut();
    expect(editor.getValue()).toBe('Drawer C');
  });
