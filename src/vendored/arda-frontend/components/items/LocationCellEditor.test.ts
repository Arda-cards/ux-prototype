...
jest.mock('@/lib/ardaClient', () => ({ lookupLocations: jest.fn().mockResolvedValue([]) }));
...
describe('LocationCellEditor', () => {
  let editor: LocationCellEditor;
  let params: {
    value: string;
  ...
  it('getGui returns a div element after init', () => {
    ...
    expect(gui.className).toContain('ag-cell-edit-location');
  });
  ...
  it('getValue returns input value when input is present', () => {
    ...
    expect(editor.getValue()).toBe('Room 1');
  });

  it('getValue returns committedValue when no input present', () => {
    editor.init(params as never);
    expect(editor.getValue()).toBe('Storage');
  });
  ...
  it('focusOut updates committedValue from input value', () => {
    ...
    gui.appendChild(input);
    editor.focusOut();
    expect(editor.getValue()).toBe('Bay 2');
  });
