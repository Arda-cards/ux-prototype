...
const NEW_FACILITY_OPTION = '__new_facility__';

interface FacilityTypeaheadProps {
  value: string;
...
export function FacilityTypeahead({
  value,
  onChange,
  ...
  const displayOptions = useMemo(() => {
    ...
    if (trimmed && !options.some((o) => o.toLowerCase() === trimmed.toLowerCase())) {
      list.push({ value: NEW_FACILITY_OPTION, isNew: true });
    }
    return list;
  }, [options, inputValue]);

  const fetchFacilities = useCallback(async (search: string) => {
    if (!search.trim()) {
      setOptions([]);
      return;
    }
    ...
    setIsLoading(true);
    try {
      const results = await lookupFacilities(search.trim());
      setOptions(results);
    } catch {
      setOptions([]);
    } finally {
  ...
  const handleFocus = useCallback(() => {
    ...
    if (inputValue.trim()) {
      fetchFacilities(inputValue);
    }
    setIsOpen(true);
    setHighlightedIndex(-1);
  }, [inputValue, fetchFacilities]);
  ...
  const selectValue = useCallback(
    (val: string) => {
      const final = val === NEW_FACILITY_OPTION ? inputValue.trim() : val;
      ...
      if (isOpen && displayOptions.length > 0) {
        ...
        if (e.key === 'Enter') {
          ...
          if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
            ...
            selectValue(NEW_FACILITY_OPTION);
          }
          return;
        }
        if (e.key === 'Escape') {
        ...
        if (e.key === 'Tab') {
          if (cellEditorMode) {
            if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
              ...
              const final = selected === NEW_FACILITY_OPTION ? inputValue.trim() : selected;
              if (final) {
                onChange(final);
                setInputValue(final);
              }
