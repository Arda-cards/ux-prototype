import { DepartmentCellEditor } from './DepartmentCellEditor';

jest.mock('@/lib/ardaClient', () => ({ lookupDepartments: jest.fn().mockResolvedValue([]) }));
...
describe('DepartmentCellEditor', () => {
  let editor: DepartmentCellEditor;
  let params: {
    value: string;
  ...
  beforeEach(() => {
    editor = new DepartmentCellEditor();
    params = {
      value: 'Cardiology',
      stopEditing: jest.fn(),
    };
  ...
  it('getGui returns a div element after init', () => {
    ...
    expect(gui.className).toContain('ag-cell-edit-department');
  });
  ...
  it('ESC keydown resets committedValue to initialValue', () => {
    editor.init(params as never);
    const gui = editor.getGui();
    const escEvent = new KeyboardEvent('keydown', { key: 'Escape', bubbles: true });
    gui.dispatchEvent(escEvent);
    // getValue should return the initial value after ESC
    expect(editor.getValue()).toBe('Cardiology');
  });

  it('getValue returns input value when input is present', () => {
    editor.init(params as never);
    const gui = editor.getGui();
    // Add an input element to simulate the rendered typeahead
    const input = document.createElement('input');
    input.value = 'Oncology';
    gui.appendChild(input);
    expect(editor.getValue()).toBe('Oncology');
  });
  ...
  it('focusOut updates committedValue from input value', () => {
    ...
    const gui = editor.getGui();
    const input = document.createElement('input');
    input.value = '  Radiology  ';
    gui.appendChild(input);
    editor.focusOut();
    expect(editor.getValue()).toBe('Radiology');
  });

  it('focusOut does nothing when wasCancelled is true', () => {
    editor.init(params as never);
    const gui = editor.getGui();
    // Press ESC to set wasCancelled (no input present, so getValue returns committedValue)
    const escEvent = new KeyboardEvent('keydown', { key: 'Escape', bubbles: true });
    gui.dispatchEvent(escEvent);
    editor.focusOut();
    // committedValue should remain initial value 'Cardiology'
    expect(editor.getValue()).toBe('Cardiology');
  });
