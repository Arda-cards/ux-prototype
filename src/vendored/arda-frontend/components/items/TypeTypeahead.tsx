...
export function TypeTypeahead({
  ...
  const displayOptions = useMemo(() => {
    ...
    if (trimmed && !options.some((o) => o.toLowerCase() === trimmed.toLowerCase())) {
      list.push({ value: NEW_TYPE_OPTION, isNew: true });
    }
    return list;
  }, [options, inputValue]);
  ...
  const fetchTypes = useCallback(async (search: string) => {
    ...
    setIsLoading(true);
    try {
      const results = await lookupTypes(search.trim());
      setOptions(results);
    } catch {
      setOptions([]);
    } finally {
  ...
  const selectValue = useCallback(
    (val: string) => {
      const final = val === NEW_TYPE_OPTION ? inputValue.trim() : val;
        ...
        if (e.key === 'Tab') {
          if (cellEditorMode) {
            if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
              ...
              const final = selected === NEW_TYPE_OPTION ? inputValue.trim() : selected;
              if (final) {
                onChange(final);
                setInputValue(final);
              }
