import { FacilityCellEditor } from './FacilityCellEditor';

jest.mock('@/lib/ardaClient', () => ({ lookupFacilities: jest.fn().mockResolvedValue([]) }));
...
describe('FacilityCellEditor', () => {
  let editor: FacilityCellEditor;
  let params: {
    value: string;
  ...
  it('getGui returns a div element after init', () => {
    ...
    expect(gui.className).toContain('ag-cell-edit-facility');
  });
  ...
  it('getValue returns input value when input is present', () => {
    ...
    expect(editor.getValue()).toBe('Clinic');
  });

  it('getValue returns committedValue when no input present', () => {
    editor.init(params as never);
    expect(editor.getValue()).toBe('Hospital');
  });
  ...
  it('focusOut updates committedValue from input value', () => {
    ...
    gui.appendChild(input);
    editor.focusOut();
    expect(editor.getValue()).toBe('Lab');
  });
