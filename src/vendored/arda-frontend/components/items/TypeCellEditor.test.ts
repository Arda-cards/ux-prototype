import { TypeCellEditor } from './TypeCellEditor';

jest.mock('@/lib/ardaClient', () => ({ lookupTypes: jest.fn().mockResolvedValue([]) }));
...
describe('TypeCellEditor', () => {
  ...
  it('getGui returns a div element after init', () => {
    ...
    expect(gui.className).toContain('ag-cell-edit-type');
  });

  it('getValue returns empty string when no input yet', () => {
    editor.init(params as never);
  ...
  it('getValue returns input value when input is present', () => {
    ...
    expect(editor.getValue()).toBe('Supply');
  });

  it('getValue returns committedValue when no input present', () => {
    editor.init(params as never);
    expect(editor.getValue()).toBe('Medical');
  });
  ...
  it('focusOut updates committedValue from input value', () => {
    ...
    gui.appendChild(input);
    editor.focusOut();
    expect(editor.getValue()).toBe('Equipment');
  });
