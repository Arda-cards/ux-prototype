import { SubTypeCellEditor } from './SubTypeCellEditor';

jest.mock('@/lib/ardaClient', () => ({ lookupSubtypes: jest.fn().mockResolvedValue([]) }));
...
describe('SubTypeCellEditor', () => {
  let editor: SubTypeCellEditor;
  let params: {
    value: string;
  ...
  it('getGui returns a div element after init', () => {
    ...
    expect(gui.className).toContain('ag-cell-edit-subtype');
  });
  ...
  it('getValue returns input value when input is present', () => {
    ...
    input.value = 'Surgical';
    gui.appendChild(input);
    expect(editor.getValue()).toBe('Surgical');
  });

  it('getValue returns committedValue when no input present', () => {
    editor.init(params as never);
    expect(editor.getValue()).toBe('Consumable');
  });
  ...
  it('focusOut updates committedValue from input value', () => {
    ...
    gui.appendChild(input);
    editor.focusOut();
    expect(editor.getValue()).toBe('Durable');
  });
