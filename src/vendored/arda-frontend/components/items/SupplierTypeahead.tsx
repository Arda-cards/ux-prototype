...
export function SupplierTypeahead({
  ...
  const displayOptions = useMemo(() => {
    ...
    if (trimmed && !options.some((o) => o.toLowerCase() === trimmed.toLowerCase())) {
      list.push({ value: NEW_SUPPLIER_OPTION, isNew: true });
    }
    return list;
  }, [options, inputValue]);
  ...
  const fetchSuppliers = useCallback(async (search: string) => {
    ...
    setIsLoading(true);
    try {
      const results = await lookupSuppliers(search.trim());
      setOptions(results);
    } catch {
      setOptions([]);
    } finally {
  ...
  const selectValue = useCallback(
    (val: string) => {
      const final = val === NEW_SUPPLIER_OPTION ? inputValue.trim() : val;
      ...
      if (isOpen && displayOptions.length > 0) {
        ...
        if (e.key === 'Enter') {
          ...
          if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
            ...
            selectValue(NEW_SUPPLIER_OPTION);
          }
          return;
        }
        if (e.key === 'Escape') {
        ...
        if (e.key === 'Tab') {
          if (cellEditorMode) {
            ...
            if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
              ...
              const final = selected === NEW_SUPPLIER_OPTION ? inputValue.trim() : selected;
              if (final) {
                onChange(final);
                setInputValue(final);
              }
