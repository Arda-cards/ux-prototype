...
const NEW_DEPARTMENT_OPTION = '__new_department__';

interface DepartmentTypeaheadProps {
  value: string;
...
export function DepartmentTypeahead({
  ...
  const displayOptions = useMemo(() => {
    ...
    if (trimmed && !options.some((o) => o.toLowerCase() === trimmed.toLowerCase())) {
      list.push({ value: NEW_DEPARTMENT_OPTION, isNew: true });
    }
    return list;
  }, [options, inputValue]);
  ...
  const fetchDepartments = useCallback(async (search: string) => {
    ...
    setIsLoading(true);
    try {
      const results = await lookupDepartments(search.trim());
      setOptions(results);
    } catch {
      setOptions([]);
    } finally {
  ...
  const selectValue = useCallback(
    (val: string) => {
      const final = val === NEW_DEPARTMENT_OPTION ? inputValue.trim() : val;
      ...
      if (isOpen && displayOptions.length > 0) {
        ...
        if (e.key === 'Enter') {
          ...
          if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
            ...
            selectValue(NEW_DEPARTMENT_OPTION);
          }
          return;
        }
        if (e.key === 'Escape') {
        ...
        if (e.key === 'Tab') {
          if (cellEditorMode) {
            if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
              ...
              const final = selected === NEW_DEPARTMENT_OPTION ? inputValue.trim() : selected;
              if (final) {
                onChange(final);
                setInputValue(final);
              }
              ...
              inputValue.trim() && (
                ...
                >
                  ...
                  <span className="text-sm text-gray-900">
                    New department: &quot;{inputValue.trim()}&quot;
                  </span>
                </div>
              )
            ) : (
