...
const NEW_USECASE_OPTION = '__new_usecase__';

interface UseCaseTypeaheadProps {
  value: string;
...
export function UseCaseTypeahead({
  value,
  onChange,
  ...
  const displayOptions = useMemo(() => {
    ...
    if (trimmed && !options.some((o) => o.toLowerCase() === trimmed.toLowerCase())) {
      list.push({ value: NEW_USECASE_OPTION, isNew: true });
    }
    return list;
  }, [options, inputValue]);

  const fetchUseCases = useCallback(async (search: string) => {
    if (!search.trim()) {
      setOptions([]);
      return;
    }
    ...
    setIsLoading(true);
    try {
      const results = await lookupUseCases(search.trim());
      setOptions(results);
    } catch {
      setOptions([]);
    } finally {
  ...
  const handleFocus = useCallback(() => {
    ...
    if (inputValue.trim()) {
      fetchUseCases(inputValue);
    }
    setIsOpen(true);
    setHighlightedIndex(-1);
  }, [inputValue, fetchUseCases]);
  ...
  const selectValue = useCallback(
    (val: string) => {
      const final = val === NEW_USECASE_OPTION ? inputValue.trim() : val;
      ...
      if (isOpen && displayOptions.length > 0) {
        ...
        if (e.key === 'Enter') {
          ...
          if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
            ...
            selectValue(NEW_USECASE_OPTION);
          }
          return;
        }
        if (e.key === 'Escape') {
        ...
        if (e.key === 'Tab') {
          if (cellEditorMode) {
            if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
              ...
              const final = selected === NEW_USECASE_OPTION ? inputValue.trim() : selected;
              if (final) {
                onChange(final);
                setInputValue(final);
              }
