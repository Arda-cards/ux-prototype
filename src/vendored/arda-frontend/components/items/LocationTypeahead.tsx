...
export function LocationTypeahead({
  ...
  const displayOptions = useMemo(() => {
    ...
    if (trimmed && !options.some((o) => o.toLowerCase() === trimmed.toLowerCase())) {
      list.push({ value: NEW_LOCATION_OPTION, isNew: true });
    }
    return list;
  }, [options, inputValue]);
  ...
  const fetchLocations = useCallback(async (search: string) => {
    ...
    setIsLoading(true);
    try {
      const results = await lookupLocations(search.trim());
      setOptions(results);
    } catch {
      setOptions([]);
    } finally {
  ...
  const selectValue = useCallback(
    (val: string) => {
      const final = val === NEW_LOCATION_OPTION ? inputValue.trim() : val;
      ...
      if (isOpen && displayOptions.length > 0) {
        ...
        if (e.key === 'Enter') {
          ...
          if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
            ...
            selectValue(NEW_LOCATION_OPTION);
          }
          return;
        }
        if (e.key === 'Escape') {
        ...
        if (e.key === 'Tab') {
          if (cellEditorMode) {
            if (highlightedIndex >= 0 && highlightedIndex < displayOptions.length) {
              ...
              const final = selected === NEW_LOCATION_OPTION ? inputValue.trim() : selected;
              if (final) {
                onChange(final);
                setInputValue(final);
              }
