...
import { VIEW_KEY_TO_FIELD, FIELD_TO_VIEW_KEY } from './itemTableConfig';
...
// Copies each intermediate object so frozen Redux state is never mutated directly.
...
function enhanceEditableColumnDefs(
  ...
  return defs.map((col) => {
    ...
    return {
      ...
      valueSetter: (params) => {
        // Shallow-copy row data — params.data is a frozen Redux object and cannot be mutated directly.
        // After applying changes to the copy we call params.node?.setData(d) so AG Grid's row node
        // reflects the new values and the cell re-renders correctly before the Redux round-trip completes.
        ...
        } else if (path === 'minQuantityUnit') {
          const item = params.data as items.Item;
          const current = item.minQuantity || { amount: 1, unit: 'each' };
          setNested(d, 'minQuantity', { ...current, unit: String(v ?? '').trim() || 'each' });
        } else if (path === 'orderQuantityAmount') {
          const item = params.data as items.Item;
          const current = item.primarySupply?.orderQuantity || { amount: 1, unit: 'each' };
          const n = typeof v === 'number' ? v : parseFloat(String(v ?? ''));
          const supply = (d.primarySupply || {}) as Record<string, unknown>;
          d.primarySupply = { ...supply, orderQuantity: { ...current, amount: isNaN(n) ? 1 : n } };
        } else if (path === 'orderQuantityUnit') {
          const item = params.data as items.Item;
          const current = item.primarySupply?.orderQuantity || { amount: 1, unit: 'each' };
          const supply = (d.primarySupply || {}) as Record<string, unknown>;
          d.primarySupply = { ...supply, orderQuantity: { ...current, unit: String(v ?? '').trim() || 'each' } };
        } else if (path === 'taxable') {
          setNested(d, path, v === 'true' || v === true || String(v).toLowerCase() === 'yes');
        } else if (path === 'primarySupply.averageLeadTime') {
          const n = typeof v === 'number' ? v : parseFloat(String(v));
          if (!isNaN(n)) {
            setNested(d, path, { length: n, unit: (params.data as items.Item).primarySupply?.averageLeadTime?.unit || 'HOUR' });
          }
        } else if (path === 'color') {
          setNested(d, path, String(v ?? '').trim() || undefined);
        } else if (path === 'primarySupply.orderMechanism') {
          const item = params.data as items.Item;
        ...
        params.node?.setData(d as unknown as items.Item);
        return true;
      },
...
export const ItemTableAGGrid = forwardRef<ItemTableAGGridRef, Props>(function ItemTableAGGrid({
        ...
        if (dropdownByRow) {
          ...
          Object.entries(dropdownByRow).forEach(([path, value]) => {
            if (path === 'primarySupply.orderMechanism' && value != null && value !== '') {
              ...
              // The taxable dropdown stores 'true'/'false' strings; the API requires a boolean.
              setNested(snap, path, value === 'true');
            } else {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // Use the instance-scoped ref to avoid targeting the wrong grid when
      // multiple grids are mounted (e.g. scan modal over items page).
      ...
      setTimeout(() => {
        ...
        columnState.forEach((col) => {
          ...
          if (colId && colId !== 'select') {
            const viewKey = FIELD_TO_VIEW_KEY[colId];
            if (viewKey) {
  ...
  useEffect(() => {
    const gridRoot = gridContainerRef.current;
    if (!gridRoot) return;
