...
const tabs = isProduction ? allTabs.filter((tab) => tab.key === 'published') : allTabs;
...
export default function ItemsPage() {
  ...
  const [itemCardsMap, setItemCardsMap] = useState<Record<string, KanbanCardResult[]>>({});
  const cardFetchPromisesRef = useRef<Partial<Record<string, Promise<KanbanCardResult[] | null>>>>(
    {},
  );
  ...
  const [itemToDuplicate, setItemToDuplicate] = useState<items.Item | null>(null);
  const [isLoadingCardsToDelete, setIsLoadingCardsToDelete] = useState(false);
  const [cardsToDeleteMap, setCardsToDeleteMap] = useState<Record<string, KanbanCardResult[]>>({});
      ...
      if (existingGridState) {
        try {
          ...
          if (gridState.columnState && Array.isArray(gridState.columnState)) {
            ...
            gridState.columnState = gridState.columnState.map((col: ColumnStateItem) => {
  ...
  const getKanbanCardsForItem = useCallback(
    async (itemEntityId: string): Promise<KanbanCardResult[] | null> => {
      ...
      try {
        ...
        devLog('[getKanbanCardsForItem] Request body:', JSON.stringify(requestBody, null, 2));

        const response = await fetch(`/api/arda/kanban/kanban-card/query-details-by-item`, {
          method: 'POST',
          headers: {
        ...
        devLog('[getKanbanCardsForItem] Response status:', response.status, response.ok);

        if (response.ok) {
          const data = await response.json();
          devLog('[getKanbanCardsForItem] Response data:', JSON.stringify(data, null, 2));
          devLog('[getKanbanCardsForItem] data.ok:', data.ok);
          devLog('[getKanbanCardsForItem] data.data:', data.data);
          devLog('[getKanbanCardsForItem] data.data.results:', data.data?.results);

          const results = data.ok ? extractKanbanRecords(data) : [];

          if (results.length > 0) {
            devLog('[getKanbanCardsForItem] Returning', results.length, 'cards');
            devLog('[getKanbanCardsForItem] First card structure:', results[0]);
            return results;
          } else {
            devWarn('[getKanbanCardsForItem] No results found in response. Structure:', {
              hasData: !!data.data,
          ...
          if (response.status !== 500) {
            console.error('[getKanbanCardsForItem] Response not OK:', response.status);
          }
  ...
  const refreshCardsForItem = useCallback(
    async (itemEntityId: string) => {
      ...
      try {
        ...
        console.error(`Error refreshing cards for item ${itemEntityId}:`, error);
      }
    },
  ...
  const fetchArdaItems = useCallback(
    async (
      ...
      try {
        ...
        devLog('[PAGINATION] Request body:', JSON.stringify(queryRequest, null, 2));
        ...
        devLog('[PAGINATION] Last item ID:', result.items[result.items.length - 1]?.entityId);
        ...
        if (result.items.length === 0 && pageIndex > 0 && !isNavigatingBackRef.current) {
          ...
          setTimeout(() => {
            fetchArdaItems(pageSize, previousPageIndex, searchQuery, false).finally(() => {
        ...
        setItemCardsMap((prev) => {
          ...
          const nextEntries = Object.entries(prev).filter(([entityId]) => allowedIds.has(entityId));
        ...
        const hasNextPage =
          ...
          Boolean(result.pagination.nextPage && result.pagination.nextPage.trim() !== '');
        const hasPreviousPage = Boolean(
          result.pagination.previousPage && result.pagination.previousPage.trim() !== '',
        );
  ...
  const refreshCurrentPage = useCallback(() => {
    const pageIndex = (paginationData.currentPage - 1) * paginationData.currentPageSize;
    fetchArdaItems(paginationData.currentPageSize, pageIndex, debouncedSearch, false);
  ...
  const handleNextPage = useCallback(() => {
    if (paginationData.hasNextPage && !loadingArdaItems) {
      ...
      const nextPageIndex = paginationData.currentPage * paginationData.currentPageSize;
      devLog(
      ...
      fetchArdaItems(paginationData.currentPageSize, nextPageIndex, debouncedSearch);
    }
  ...
  const handlePreviousPage = useCallback(() => {
    if (paginationData.hasPreviousPage && !loadingArdaItems) {
      ...
      fetchArdaItems(paginationData.currentPageSize, previousPageIndex, debouncedSearch);
    }
  }, [paginationData, fetchArdaItems, loadingArdaItems, debouncedSearch]);
  ...
  }, [paginationData.currentPageSize, fetchArdaItems, loadingArdaItems, debouncedSearch]);
  ...
  useEffect(() => {
    const handleItemDeleted = () => {
      ...
      const currentPageIndex = (paginationData.currentPage - 1) * paginationData.currentPageSize;
      fetchArdaItems(paginationData.currentPageSize, currentPageIndex, debouncedSearch);
    };
  ...
  useEffect(() => {
    const onPopState = () => {
      ...
      const onItems =
        path === '/items' || path.startsWith('/items/') || /^\/item\/[^/]+$/.test(path);
      if (onItems) return;
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      try {
        ...
        const onItems =
          path === '/items' || path.startsWith('/items/') || path.match(/^\/item\/[^/]+$/);
        if (onItems) return;
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const displayedItems = itemsGridRef.current?.getDisplayedItems?.() ?? ardaItems;
      if (displayedItems.length === 0) return;
      const direction = event.key === 'ArrowDown' ? 'down' : 'up';
      const next = getAdjacentItem(displayedItems, selectedItem.entityId, direction);
      if (next) {
        event.preventDefault();
      ...
      const cardFetchPromises = itemsToDelete.map(async (item) => {
        try {
          ...
          if (response.ok) {
            ...
            console.error(`Failed to fetch cards for item ${item.entityId}:`, response.status);
            cardsMap[item.entityId] = [];
          }
        } catch (error) {
          console.error(`Error fetching cards for item ${item.entityId}:`, error);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      for (const item of itemsToDeleteList) {
        ...
        if (cardsToDelete.length > 0) {
          const cardDeletePromises = cardsToDelete.map(async (card) => {
            try {
              const response = await fetch(`/api/arda/kanban/kanban-card/${card.payload.eId}`, {
                method: 'DELETE',
                headers: {
                  'Content-Type': 'application/json',
                  Authorization: `Bearer ${jwtToken}`,
      ...
      if (successful.length === itemsToDeleteList.length) {
        const cardMessage =
          totalCardsDeleted > 0
            ? ` and ${totalCardsDeleted} associated card${totalCardsDeleted > 1 ? 's' : ''}`
            : '';
        toast.success(
        ...
        const cardMessage =
          totalCardsDeleted > 0
            ? ` (${totalCardsDeleted} card${totalCardsDeleted > 1 ? 's' : ''} deleted)`
            : '';
        toast.warning(
  ...
  const handlePublishAndAddAnotherFromEdit = async () => {
    ...
    const currentPageIndex = (paginationData.currentPage - 1) * paginationData.currentPageSize;
    await fetchArdaItems(paginationData.currentPageSize, currentPageIndex, debouncedSearch);
  };
    ...
    try {
            ...
            if (data.ok && records.length > 0) {
              ...
              records.forEach(
                (result: {
                  ...
                  const template = result.payload.itemDetails?.cardSize || 'UNKNOWN';
      ...
      for (const [template, cardIds] of cardsByTemplate.entries()) {
        const printPromise = fetch('/api/arda/kanban/kanban-card/print-card', {
          ...
          .then(async (printResponse) => {
            if (printResponse.ok) {
              ...
              if (
                ...
                const errorMessage =
                  errorData?.data?.responseMessage || errorData?.error || 'Failed to print cards';
          ...
          .catch((error) => {
            errorCount += cardIds.length;
            console.error(`Error printing cards for template ${template}:`, error);
    ...
    try {
      ...
      for (const item of selectedItems) {
        try {
          ...
          console.error(`Error fetching record ID for item ${item.entityId}:`, error);
        }
      }
      ...
      if (printResponse.ok) {
        ...
        if (data.ok && data.data && data.data.url) {
          ...
          console.error('Failed to print labels - invalid response structure:', data);
          toast.error('Failed to print labels - invalid response');
        }
      } else {
        const errorData = await printResponse.json().catch(() => ({}));
        console.error('Failed to print labels:', printResponse.status, errorData);
        ...
        if (
          ...
          const errorMessage =
            errorData?.data?.responseMessage || errorData?.error || 'Failed to print labels';
          toast.error(errorMessage);
        }
      }
    } catch (error) {
  ...
  const handlePrintSelectedBreadcrumbs = async () => {
    ...
    try {
      ...
      const printResponse = await fetch('/api/arda/item/item/print-breadcrumb', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${jwtToken}`,
      ...
      if (printResponse.ok) {
        ...
        if (data.ok && data.data && data.data.url) {
          ...
          console.error('Failed to print breadcrumbs - invalid response structure:', data);
          toast.error('Failed to print breadcrumbs - invalid response');
        }
      } else {
        const errorData = await printResponse.json().catch(() => ({}));
        console.error('Failed to print breadcrumbs:', printResponse.status, errorData);
        ...
        if (
          ...
          const errorMessage =
            errorData?.data?.responseMessage || errorData?.error || 'Failed to print breadcrumbs';
          toast.error(errorMessage);
        }
      }
    } catch (error) {
            ...
            if (data.ok && records.length > 0) {
              ...
              const mappedCards = records.map(
                (result: {
                  ...
                  minUnit: item.minQuantity?.unit || 'unit',
                  location: item.locator?.location || '',
                  orderQty: result.payload.cardQuantity?.amount?.toString() || '',
    ...
    <SidebarProvider>
      ...
      <SidebarInset className="overflow-hidden">
        ...
        <div className="w-full px-10 pt-4 mt-16 md:px-8 md:pt-6 flex flex-col gap-4 flex-1 min-h-0 overflow-hidden">
          <div>
            <h1 className="text-[24px] font-semibold leading-8 text-black font-inter">Items</h1>
          ...
          {loadingArdaItems && isInitialLoadRef.current ? (
            <div className="border border-dashed border-border rounded-xl p-4 flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <h2 className="text-base font-semibold text-foreground">Loading items...</h2>
                ...
                <div className="flex flex-wrap gap-2">
                  <DropdownMenu
                    ...
                    onOpenChange={(open) => {
                      ...
                      if (open) setColumnVisibilityDraft({ ...columnVisibility });
                    }}
                  >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="min-w-[220px]">
                      ...
                      <DropdownMenuItem
                        onClick={() => {
                          if (selectedItems.length === 1 && selectedItems[0].entityId) {
                            setSelectedItem(selectedItems[0]);
                            setManagePanelOpen(true);
                      ...
                      >
                        <span className="flex-1 flex items-center gap-2">
                          {isPrintingCards ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                          {isPrintingCards ? 'Printing cards...' : 'Print cards…'}
                        </span>
                        {/* <span className='text-xs text-muted-foreground'>
                          ⌘P
                        </span> */}
                      ...
                      >
                        <span className="flex-1 flex items-center gap-2">
                          {isPrintingLabels ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                          {isPrintingLabels ? 'Printing labels...' : 'Print labels…'}
                        </span>
                      </DropdownMenuItem>
                      <DropdownMenuItem
                      ...
                      >
                        <span className="flex-1 flex items-center gap-2">
                          ...
                          {isPrintingBreadcrumbs ? 'Printing breadcrumbs...' : 'Print breadcrumbs…'}
                        </span>
                      </DropdownMenuItem>
                      <DropdownMenuItem
                      ...
                      >
                        <span className="flex-1 flex items-center gap-2">
                          {isLoadingPreview ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                          {isLoadingPreview ? 'Loading cards...' : 'Preview card(s)'}
                        </span>
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onClick={() => {
                          if (selectedItems.length === 0) {
                            toast.error('Please select at least one item to duplicate');
                            return;
                          }
                          if (selectedItems.length > 1) {
                            toast.error('Please select only one item to duplicate');
                            return;
                          }
              ...
              <div className="flex-1 w-full min-h-0">
                <ItemTableAGGrid
                  ...
                  onOpenItemDetails={(item: items.Item) => {
                    if (!item?.entityId) {
                      toast.error('Cannot open item details: missing entity ID');
                      return;
                    }
                    setSelectedItem(item);
                    setManagePanelOpen(true);
          ...
          <ItemFormPanel
            ...
            onPublishAndAddAnotherFromAddItem={handlePublishAndAddAnotherFromAddItem}
            onSuccess={async () => {
              const currentPageIndex =
                (paginationData.currentPage - 1) * paginationData.currentPageSize;
              ...
              if (entityIdToRefresh && Array.isArray(freshItems)) {
                const updatedItem = freshItems.find((item) => item.entityId === entityIdToRefresh);
                if (updatedItem) {
