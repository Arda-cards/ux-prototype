...
- **Revert-on-failure for auto-publish** — `useRowAutoPublish` snapshots `oldValue` per `(rowId, field)` on the first `cellValueChanged`; on a publish failure the hook rolls the row's cells back via `api.applyTransaction({ update: [row] })` and clears pending/dirty/snapshot so the row returns to `'idle'`. `ConnectedDataGrid` wires `getApi` automatically.
