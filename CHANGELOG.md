...
### Fixed
- **ImageDisplay: no stale error badge when `imagePending` clears on the same URL** — `loadState` was reset only when `imageUrl` changed, so a failure that occurred before a pending window survived it: clearing `imagePending` re-mounted the `<img>` but rendered the previous error badge and initials until the new request resolved, which is misleading during a retry. The load state now resets when the pending flag flips too.
...
- **MultiSelectTypeaheadInput: keyboard twins for the hover-revealed buttons** — Shift+Delete on a highlighted dropdown row fires `optionDestroy`, Shift+Space on a focused token fires `tokenAction`, and Shift+Enter on a focused token edits it (with `editOnDoubleClick`). Announced via `aria-keyshortcuts`; documented in the Keyboard section. A dev-time `console.warn` also flags the unsupported `bare` + `cellEditorMode` combination.
...
### Fixed
- **MultiSelectTypeaheadInput: token identity is case-insensitive on every path** — choosing/toggling a dropdown option now dedupes case-insensitively, matching the create and outside-click paths (previously `alice@x.com` + picking `Alice@x.com` produced duplicate tokens); the dropdown checkmark agrees.
- **TokenChip: `transition-all` → explicit `transition-[margin,width,opacity]`** — per the animate-transforms-and-opacity-only rule.
...
- **Toggle** atom — two-state on/off button with `default` and `outline` variants
- **ToggleGroup** atom — single or multiple selection toggle set with shared variant/size context
- **ButtonGroup** atom — CSS-only layout with border collapsing, `showDivider` auto-insertion, and orientation support
- **SplitButton** atom — primary action + dropdown chevron composing Button, ButtonGroup, and DropdownMenu
